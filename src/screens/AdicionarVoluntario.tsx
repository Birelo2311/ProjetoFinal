...
type AdicionarVoluntarioNavigationProp = NativeStackNavigationProp<
  RootStackParamList,
  'AdicionarVoluntario'
>;

type Props = {
  navigation: AdicionarVoluntarioNavigationProp;
};

export default function AdicionarVoluntario({ navigation }: Props) {
  ...
  const verificarCPFExistente = async (cpfSemMascara: string) => {
    const snapshot = await firestore()
      .collection('voluntarios')
      .where('cpf', '==', cpfSemMascara)
      .get();
  ...
  const handleSalvar = async () => {
    ...
    if (await verificarCPFExistente(cpfSemMascara)) {
      return Alert.alert('Erro', 'CPF já cadastrado.');
    }
    ...
    try {
      await firestore().collection('voluntarios').add({
        nome,
        telefone,
        cpf: cpfSemMascara,
        cep: cepSemMascara,
      ...
      Alert.alert('Sucesso', 'Voluntário cadastrado com sucesso!');
      navigation.goBack();
    } catch (error) {
      Alert.alert('Erro', 'Erro ao salvar voluntário.');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.title}>Adicionar Voluntário</Text>
